...
class FirstNN(object):
    ...
    def __init__(self, input_dims, num_nodes_lr1, num_classes, param_init='small_std', std=1e-4):
        ...
        if param_init == 'small_std':
            self.params['W1']=np.random.normal(0,0.01,size=(input_dims,num_nodes_lr1))
            self.params['W2']=np.random.normal(0,0.01,size=(num_nodes_lr1,num_classes))
        elif param_init == 'ninn_std':
            self.params['W1']=np.random.normal(0,1/np.sqrt(input_dims),size=(input_dims,num_nodes_lr1))
            self.params['W2']=np.random.normal(0,1/np.sqrt(num_nodes_lr1),size=(num_nodes_lr1,num_classes))
            
        elif param_init == 'Xavier':
            self.params['W1']=np.random.normal(0,2/np.sqrt(input_dims+num_nodes_lr1),size=(input_dims,num_nodes_lr1))
            self.params['W2']=np.random.normal(0,2/np.sqrt(num_nodes_lr1+num_classes),size=(num_nodes_lr1,num_classes))
    ...
    def forword(self, X):
        ...
        # shape (N, C).                                                             #
        #############################################################################
        hid_act=sigmoid_func(np.dot(X,self.params['W1'])+self.params['b1'])
        prob_scores=np.exp(np.dot(hid_act,self.params['W2'])+self.params['b2'])/np.sum(np.exp(np.dot(hid_act,self.params['W2'])+self.params['b2']),axis=1).reshape(-1,1)
    ...
    def loss(self, Y, prob_scores):
        ...
        #############################################################################
        N=Y.shape[0]
        cor_prob=prob_scores[np.arange(N),Y]
        loss=-np.sum(np.log(cor_prob))/N
    ...
    def backword(self, X, Y, prob_scores):
        ...
        dZ2[np.arange(N),Y]-=1
        dZ2/=N
        grads['W2']=np.dot(sigmoid_func(np.dot(X,self.params['W1'])+self.params['b1']).T,dZ2)
        grads['b2']=np.sum(dZ2,axis=0)
        dZ1=np.dot(dZ2,self.params['W2'].T)*sigmoid_func(np.dot(X,self.params['W1'])+self.params['b1'])*(1-sigmoid_func(np.dot(X,self.params['W1'])+self.params['b1']))
    ...
    def optimizer(self, grads, update_rule='gd'):
        ...
        if self.update_rule == 'gd':
            for param in self.params:
                self.params[param]-=self.learning_rate*grads[param]
            
        elif self.update_rule == 'm_gd':
            for param in self.params:
                self.vel[param]=self.beta*self.vel[param]+self.learning_rate*grads[param]
                self.params[param]+=self.vel[param]
                  
        elif self.update_rule == 'nm_gd':
            for param in self.params:
                self.vel[param]=self.beta*self.vel[param]-self.learning_rate*grads[param]
                self.params[param]+=self.beta*self.vel[param]-self.learning_rate*grads[param]
    ...
    def predict(self, X, best_param=False):
        ...
        ###########################################################################
        if best_param:
            W1=self.best_params['W1']
            W2=self.best_params['W2']
            ...
            b1=self.params['b1']
            b2=self.params['b2']
        hid_act=sigmoid_func(np.dot(X,W1)+b1)
        prob_scores=np.exp(np.dot(hid_act,W2)+b2)/np.sum(np.exp(np.dot(hid_act,W2)+b2),axis=1).reshape(-1,1)
